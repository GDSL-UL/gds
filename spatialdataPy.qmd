...
::: callout-important
You need the `data/London` folder on your computer, read locally. To do that:

1.  Download the `London` folder by right-clicking [on this link](https://github.com/GDSL-UL/gds/tree/main/data/London) and saving the file (or use the "Download a folder on github" trick from the OpenScience session: <https://download-directory.github.io/>)

2.  Place it inside the `data` folder you created in the previous Lab, so you end up with `data/London/Polygons`, `data/London/Lines`, and `data/London/Tables`.
:::
...
We can also import point files. So far, we have imported `shapefiles` and `geojsons` for polygons and lines — the same works for points. Here we have a `bus_stops` shapefile for London.

```{python}
bus_stops = gpd.read_file("data/London/Points/bus_stops.shp")

bus_stops.plot(markersize=3, color="dodgerblue", alpha=0.6)
plt.show()
```

*Note*: point data can also come from other sources like **OpenStreetMap**, queried live with the `osmnx` package in `Python` or `osmdata` in `R` — we won't use that route here, but it's worth knowing it exists for finding point data that isn't already packaged as a shapefile.
:::
...
*(the line below reprojects `poi_gdf` — we'll properly cover what CRSs are and why this is needed in the [Coordinate reference Systems] section below; for now, just note that `districts` and `poi_gdf` need to share the same CRS before we can plot them together)*
...
# Plot points of interest with blue color and adjusted size
...
::: callout-important
Don't confuse `.to_crs()` with `.set_crs()`. `.to_crs()` **reprojects** data: it mathematically transforms every coordinate, and only makes sense if the `GeoDataFrame` already has a *correct* CRS set. `.set_crs()` **relabels** data: it tells `geopandas` what CRS the existing coordinates are already in, without moving a single point — that's what you want when the CRS is missing or was set incorrectly (we already used it this way earlier, on `poi_gdf`). Using `.to_crs()` on data with a missing or wrong CRS won't fix it — at best it errors, at worst it silently transforms your coordinates as if the wrong CRS were correct, distorting the data.
:::
...
## rename(columns={'Zone_Code': 'DIST_CODE'}) specifies that the column name Zone_Code should be renamed to DIST_CODE
...
- We use `how='left'` to keep every district (matching the R lab, which uses `left_join()`) — the default `merge()` behaviour is an inner join, which would silently drop any district with no matching qualifications row.
...
```{python}
district_qual = districts.merge(qualifications2001_df, on='DIST_CODE', how='left')
# Prove it worked
...
::: callout-important
Two separate gotchas with free basemap tiles, both of which show up as a "blocked"/watermarked tile instead of an actual map:

-   **CARTO** retired free, unauthenticated access — providers like `cx.providers.CartoDB.Voyager`, `.Positron` and `.DarkMatter` now return a tile stamped "API KEY REQUIRED" unless you configure your own CARTO API key.
-   **OpenStreetMap's own tiles** (`cx.providers.OpenStreetMap.Mapnik`) are free, but OSM's tile usage policy actively blocks scripted/bulk access that doesn't properly identify itself — exactly what a classroom full of students all requesting tiles via `contextily` at once looks like from their end. You'll see a "403 Access blocked" tile with a link to `osm.wiki/Blocked`.

**Esri's** tiles are still immediately available — `cx.providers.Esri.WorldStreetMap` and `cx.providers.Esri.WorldImagery` (both used below) stay free, key-free, and don't enforce the same anti-bulk-access policy.
:::

We can begin by creating a map in the same way we would do normally, and then use the `add_basemap` command to add a basemap. We're explicit about `Esri.WorldStreetMap` here since it doesn't require an API key and isn't blocked for bulk classroom use:

```{python}
ax = bus_stops.plot(markersize=3, color="black")
cx.add_basemap(ax, crs=bus_stops.crs, source=cx.providers.Esri.WorldStreetMap);
# Show the plot
plt.show()
```
