...
The labs for this session cover the same ground in both languages. Here
is how the key functions line up, so you can follow either lab (or
switch between them) without losing the thread:

| Task | R | Python |
|----|----|----|
| Pull coordinates out of a spatial object | `st_coordinates()` | `.geometry.x` / `.geometry.y` |
| Kernel density estimation | `geom_density_2d_filled()` | `sns.kdeplot(fill=True)` |
| Adjust KDE bandwidth | `adjust = 0.5` / `2` | `bw_adjust = 0.5` / `2` |
| Density-based clustering | `dbscan::dbscan(eps = 100, minPts = 50)` | `DBSCAN(eps=100, min_samples=50)` |
| Find the optimal `eps` | `kNNdistplot(coords, k = 50)` | `NearestNeighbors(n_neighbors=50)` + sorted k-distance plot |
| More robust clustering variant | `hdbscan()` | `ADBSCAN()` |

::: callout-note
The last row is the one place the two labs genuinely diverge.
**HDBSCAN** (R) and **A-DBSCAN** (Python) share the same motivation —
handling clusters of varying density more robustly than plain DBSCAN —
but they are different algorithms and work differently under the hood.
Each lab flags this where it comes up. Everything else in the table
produces comparable output across the two languages.
:::
