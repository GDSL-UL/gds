...
Once you have read through, jump on the [Do-It-Yourself](openscienceDIY.html) section, which will provide you with a challenge that you should complete on your own, and will allow you to put what you have already learnt into practice.
...
1.  Download the `census2021_ethn` file by right-clicking [on this link](https://github.com/GDSL-UL/gds/tree/main/data) and saving the file
...
2.  Then go to the folder you need to today. So for example copy: https://github.com/GDSL-UL/gds/tree/main/data/London
...
We can check the top (bottom) X lines of the table by passing X to the method head (tail). For example, for the top five lines:

```{python}
census2021.head() # read first 5 rows
```

And the bottom five lines:
...
::: callout-tip
Jupyter (and Quarto) only display the result of the *last* expression in a code cell, so `head()` and `tail()` need separate cells if you want to see both outputs — unlike R, which prints every top-level visible expression in a chunk.
:::
