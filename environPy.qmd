...
On a university machine, choose the **University machine** tab in each step, and see [University machines](#uni-troubleshooting) in Troubleshooting for the two problems specific to those computers.
...
You don't install anything at this step: **Anaconda** is already installed on most university machines, and it provides the same `conda` command.

1.  Choose a machine where Anaconda is installed, and **always use the same one** (e.g. CT60, Station 17, Orange Zone). The environment is installed on that machine only, so on a different machine you would have to install it again.
2.  If Anaconda isn't installed, get it from **Install University Applications** (search for `Anaconda`).
3.  Search for **Anaconda Prompt** (or **Anaconda Powershell Prompt**) in the Start menu. This is the terminal you will use for everything on this page.
4.  Keep your course folder on your `M:\` drive, so your work is available from any machine.

If `conda` asks you to accept Anaconda's Terms of Service, see [Troubleshooting](#troubleshooting-py).
...
Search for **Anaconda Prompt** (or **Anaconda Powershell Prompt**) in the Start menu and open it.
...
Your course folder is on the `M:\` drive, so the `/d` is needed to switch from `C:` to `M:`. Check you are in the right place by running `dir`: it should list `envs363_563.yml` and your `data` folder.
...
3.  Open your terminal (**Miniforge Prompt** on Windows, **Anaconda Prompt** on a university machine, **Terminal** on macOS) and move into your course folder with `cd`, as in step 3 of [Install the course environment](#install-the-course-environment).
...
4.  Activate the environment and start **Jupyter Notebook**:
    ...
    On a university machine, `jupyter notebook` may be blocked by the university's security policy. If it is, start it this way instead:
...
::: callout-caution
## Working on a university machine?

Two things work differently on university-managed computers:

-   `jupyter notebook` may be blocked by the Device Guard security policy. Start Jupyter Notebook with `python -m notebook` instead ([details](#uni-jupyter-blocked)).
-   Jupyter Notebook may open in the wrong folder and not show your `M:\` files ([how to fix it](#uni-jupyter-folder)).
:::
...
### University machines {#uni-troubleshooting}

These two problems only happen on university-managed computers.

::: {.callout-note collapse="true" #uni-jupyter-blocked}
## University machine: `jupyter notebook` is blocked

On university-managed computers, the Device Guard security policy can block the `jupyter notebook` command, so it refuses to start. Start Jupyter Notebook through Python instead, which works:
...
Everything else stays the same: `cd` into your course folder first, and keep the terminal open while you work.
:::

::: {.callout-note collapse="true" #uni-jupyter-folder}
## University machine: Jupyter Notebook opens in the wrong folder, or can't see my `M:\` files

This happens when Jupyter Notebook is started from somewhere other than your course folder, for example from a Start menu shortcut or Anaconda Navigator.

Always start it from the terminal, after moving into your course folder:
...
If you still can't see your files, tell Jupyter Notebook which folder to open:

``` bash
jupyter notebook --notebook-dir=M:\envs363_563
```

(or `python -m notebook --notebook-dir=M:\envs363_563` if `jupyter notebook` is blocked)

To set that folder permanently on a machine you always use, create a configuration file:

1.  In the **Anaconda Prompt**, run `jupyter server --generate-config`. This creates `C:\Users\username\.jupyter\jupyter_server_config.py`.
2.  Open that file in a text editor (e.g. [Notepad++](https://notepad-plus-plus.org)), search for `c.ServerApp.root_dir`, remove the `#` at the start of the line, and set it to your course folder, doubling each backslash: `c.ServerApp.root_dir = 'M:\\envs363_563'`.
3.  Save the file and close it. Jupyter Notebook will now always open in that folder.

![Editing the Jupyter configuration file](img/Step4_py.PNG){width="80%"}
:::
