...
::: callout-note
The AHAH dataset was developed by the Geographic Data Service (GeoDS), part of Smart Data Research UK, based at UCL, the University of Liverpool, the University of Oxford and the University of Edinburgh. If you want to find out more about the background and details of the project, have a look at the [information page](https://data.geods.ac.uk/dataset/access-to-healthy-assets-hazards-ahah) at the GeoDS website.
:::

We are going to use the Access to Healthy Assets and Hazards (AHAH) index. This is a score that ranks LSOAs (the same polygons we used in the [spatial DIY section](spatialdataDIY.html)) by the proximity to features of the environment that are considered positive for health (assets) and negative (hazards). The resulting number gives us a sense of how "unhealthy" the environment of the LSOA is. The higher the score, the less healthy the area is assessed to be.
...
You'll need to log in (registration is free) to download. Find the **"Access to Healthy Assets and Hazards (AHAH) v2 Data Pack for Liverpool (E08000012)"** resource on that page and download it.
:::
...
lsoas = gpd.read_file("data/Liverpool/Access_to_Healthy_Assets_and_Hazards_AHAH/Local_Authority_Districts/E08000012/shapefiles/E08000012.shp")
```
:::
