...
```{r message=FALSE}
library(tidyverse) # core package for data manipulation, including dplyr, readr, and more
```
...
tail(census2021) # read last 5 rows
```
...
One shot, using the package [`dplyr`](https://www.tidyverse.org/packages/) (part of the tidyverse!). More info can be found [here](https://dplyr.tidyverse.org/):
...
We can select with `c()`. If this structure is new to you have a look [here](environR.html).
