...
```{r}
bus_stops <- read_sf("data/London/Points/bus_stops.shp")

plot(bus_stops$geometry, pch = 16, col = adjustcolor("dodgerblue4", alpha.f = 0.6), cex = 0.6)
```

*Note*: point data can also come from other sources like **OpenStreetMap**, queried live with the `osmdata` package in `R` or `osmnx` in `Python` — we won't use that route here, but it's worth knowing it exists for finding point data that isn't already packaged as a shapefile.
:::
...
::: callout-note
Go back to [open science](openscienceR.html) for subsetting with `dplyr`.
:::
...
::: callout-tip
Note that `poi_sf` (created earlier with CRS `EPSG:4326`) is being plotted directly alongside `districts` (which is in the British National Grid, `EPSG:27700`) — no reprojection step first, unlike the Python lab, which had to call `.to_crs()` before combining layers. `ggplot2`'s `geom_sf()` automatically reprojects every layer to match the CRS of the first one added, so mismatched CRSs "just work" in `R`. `matplotlib`/`geopandas`'s `.plot()` does **not** do this — mismatched CRSs there will silently misalign, which is why the Python lab reprojects explicitly.
:::
...
::: callout-important
Don't confuse `st_transform()` with `st_set_crs()`. `st_transform()` **reprojects** data: it mathematically transforms every coordinate, and only makes sense if the object already has a *correct* CRS set — that's what the code below demonstrates. `st_set_crs()` **relabels** data: it tells `sf` what CRS the existing coordinates are already in, without moving a single point — that's what you want when the CRS is missing or was set incorrectly (we'll use it that way in the next section, on `housesales`). Using `st_transform()` on data with a missing or wrong CRS won't fix it — at best it errors, at worst it silently transforms your coordinates as if the wrong CRS were correct, distorting the data.
:::

```{r}
districts_4326 = st_transform(districts, "EPSG:4326") # reproject to WGS84
...
We want to transform the .csv in a `sf` object with the `st_as_sf` function using the coordinates stored in the `greastings` and `grnorthing` columns, and then we set the `dataframe` CRS to the British National Grid (EPSG:27700) using the `st_set_crs` function.
...
housesales_sf <- st_as_sf(housesales_filtered, coords = c("greastings", "grnorthing")) # denote columns which have the coordinates
housesales_clean <- st_set_crs(housesales_sf, 27700)# set crs to British National Grid 
```

As we've seen in [open science](openscienceR.html), we can do consecutive operations using `dplyr` pipes `%>%`, they are used to simplify syntax. Pipes allow to perform successive operations on dataframes in one command! More info [here](https://seananderson.ca/2014/09/13/dplyr-intro/).
...
housesales_clean = housesales %>% # select the main object
  ...
  st_as_sf(coords = c("greastings", "grnorthing")) %>% # denote columns which have the coordinates
...
Here, we buffer every district's centroid with the `st_buffer()` function from the `sf` package — for example, to draw a 1km buffer around each one:
...
::: callout-important
Some basemaps like the ones made by CARTO retired free, unauthenticated access to the tiles behind `dark`, `positron`, `voyager` and `hydda` — using any of those without your own CARTO API key now returns a tile stamped "API KEY REQUIRED" instead of an actual map. Stick to `mapnik` (plain OpenStreetMap tiles) or `wikimedia`, which stay free and key-free. The `google-*` styles need their own Google Maps API key set up separately, regardless of this issue.
:::

We simply add base_map () to our ggplot. We're explicit about `basemap = "mapnik"` here since it doesn't require an API key:

```{r}
ggplot() +
  base_map(st_bbox(districts_4326), basemap = "mapnik", increase_zoom = 2) + 
