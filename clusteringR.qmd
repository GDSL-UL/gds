...
map_list <- vector("list", length(ratings))

for (i in seq_along(ratings)) {
  var   <- ratings[i]
  title <- ratings_names[i]
...
# Bigger overall maps in the grid
wrap_plots(map_list, nrow = 3, ncol = 3) &
...
# Ensure k5cls is categorical (factor)
abb$k5cls <- factor(abb$k5cls)

map_cluster_gg <- ggplot(abb) +
  geom_sf(aes(fill = k5cls), color = "white", linewidth = 0.01) +
  
  # ---- DISCRETE viridis palette ----
  scale_fill_viridis_d(option = "viridis", name = "Clusters") +
...
abb$regionalisation <- factor(abb$regionalisation)

map_cluster_gg <- ggplot(abb) +
  geom_sf(aes(fill = regionalisation),
          color = "white",
          linewidth = 0.01) +
...
# ggmap requires WGS84
abb_regions <- st_transform(abb_regions, 4326)

bb <- st_bbox(abb_regions)

map_regions_ggmap <- ggplot(abb_regions) +
  # borders only (no fill)
  geom_sf(fill = NA, color = "red", linewidth = 1.5) +
...
```{r}
map_regions_boroughs_gg <- ggplot() +
  # regions in red
  geom_sf(data = abb_regions,
          fill = NA,
          color = "red",
          linewidth = 1.5) +
  # boroughs in green
